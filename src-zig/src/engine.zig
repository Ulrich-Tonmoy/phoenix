...
pub const Engine = struct {
    ...
    pub fn isRunning(self: *Self) bool {
        ...
        gl.clearColor(toFloat1(212), toFloat1(25), toFloat1(125), 1);
    ...
    pub const Input = struct {
        ...
        pub fn keyAction(self: Input, key: glfw.Key, action: glfw.Action) bool {
            for (self.keyEvents.constSlice()) |e| {
                return e.key == key and e.action == action;
            }
...
pub const Scene = struct {
    ...
    pub fn addGameObject(self: *Scene, mesh: *Mesh, material: *Material) !*GameObject {
        var gameObj = try self.gameObjects.addOne();
        gameObj.* = .{ .mesh = mesh, .material = material };
        return gameObj;
    }
...
pub const Texture = struct {
    ...
    pub fn log(self: Texture) void {
        std.log.info("width: {}, height: {}, channels: {}, isValid: {}", .{
            self.width,
            self.height,
            self.channels,
            self.buffer != null,
