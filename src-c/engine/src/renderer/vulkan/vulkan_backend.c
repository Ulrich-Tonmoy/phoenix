...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    for (u32 i = 0; i < required_validation_layer_count; ++i) {
        PINFO("Searching for layer: %s...", required_validation_layer_names[i]);
        b8 found = FALSE;
        ...
        if (!found) {
            PFATAL("Required validation layer is missing: %s", required_validation_layer_names[i]);
            return FALSE;
        }
    }
